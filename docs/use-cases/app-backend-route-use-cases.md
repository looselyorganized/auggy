...
| Webhooks and callbacks | `POST /webhooks/stripe`, `POST /webhooks/clerk`, `GET /visitor-auth/verify` | `webhook.signature("stripe", ...)` or `webhook.signature("svix", ...)`; providers without a verifier remain handler-verified | External systems update state without waking the model. Failures can notify the operator. |
