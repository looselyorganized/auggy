...
| Webhooks and callbacks | `POST /webhooks/stripe`, `POST /webhooks/calendar`, `GET /visitor-auth/verify` | `webhook.signature("stripe", ...)` for Stripe; unsupported policy providers fail startup until their verifiers land | External systems update state without waking the model. Failures can notify the operator. |
