...
| Webhooks and callbacks | `POST /webhooks/stripe`, `POST /webhooks/calendar`, `GET /visitor-auth/verify` | Today `none` plus handler verification; future `webhook.signature` | External systems update state without waking the model. Failures can notify the operator. |
