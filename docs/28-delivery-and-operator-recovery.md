...
| AgentMail inbound | REST catch-up, WebSocket, and Svix delivery converge on `AMIL/v5`. A message is durably claimed before kernel admission; an expired claim is fenced, never leased again. Exact creator-attention generations feed immutable metadata-only digest batches, with compact retirement ranges preserving generation continuity. Durable quota evidence charges one unique admitted message across retry and restart; every pre-model policy rejection becomes a bounded content-free tombstone backed by a fixed-size fail-closed replay filter. | Explicit unknown results, unknown exceptions, lost completion records, expired leases, and interrupted processing become durable incidents. Every later message in that provider thread is blocked, and its scheduler lane is restored on restart. A pending digest target cannot be changed and re-keyed. | Creator action compares incident ID and version, records only an evidence hash, and chooses already-handled or confirmed-no-effect. Digest presentation/dismissal uses generation CAS, acknowledges the matching Notify operation after durable settlement, and never mutates mail attention. Core releases the lane only after every durable authority for that runtime thread is clear. |
...
`AUID/v2`, `DJOB/v2`, `TGRP/v2`, `AMIL/v5`, and `NTFY/v2` are replay-critical. Back them
