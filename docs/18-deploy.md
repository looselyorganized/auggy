...
11. **Index write** — your `~/.auggy/agents.json` gets a `cloud` record so subsequent `auggy deploy zip` runs are idempotent redeploys.
...
- **Cross-machine `~/.auggy/agents.json` sync.** Each developer machine has its own index. Cloud deployment doesn't sync state back.
