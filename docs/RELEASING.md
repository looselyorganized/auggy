...
uses the explicit `release/*` namespace. The branch rule handles the one-time
unpublished `0.5.0` already on `main`; ordinary feature PRs still run the full
test and packed-release smoke without contacting npm for release admission.
...
```bash
# After the release/0.5.0-rc.1 PR and all RC gates pass:
git checkout main && git pull
git tag v0.5.0-rc.1             # tag MUST match all six package versions
git push origin v0.5.0-rc.1
# CI publishes to npm under next and creates a GitHub prerelease.
```
...
- [ ] Set the same exact version in all six publishable package manifests. For
  an RC, use the exact prerelease such as `0.5.0-rc.1` and update internal
...
Until the Environment and all six trusted publishers are configured,
publishing intentionally fails closed. This GitHub/npm state cannot be
completed or verified by a source PR.
