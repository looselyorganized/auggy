...
`0.5.0-rc.8` is the pending release-PR candidate; it is not published and must
...
all six `0.5.0-rc.7` packages, so the RC.7 provenance gate is complete. RC.8,
...
closed. RC.8, stable `0.5.0`, and every later candidate require fresh
