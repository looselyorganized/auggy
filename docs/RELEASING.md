...
token restrictions are configured. The remaining sequence is to publish
`0.5.0-rc.1` under `next`, prove its OIDC provenance and post-publish behavior,
then publish final `0.5.0` under `latest` after the RC gate passes.

Do not claim package provenance until npm displays the attestation generated by
the first OIDC publication. All publishable package manifests contain the exact
public GitHub `repository` URL required by npm trusted publishing.
...
trusted publishing. The provenance gate is not complete until the first RC
publication displays npm's generated attestation for all six packages. Final
`0.5.0` remains blocked on that evidence.
...
secret migration were completed on 2026-07-28. Confirm the obsolete npm account
token itself is revoked before tagging. npm does not validate trusted-publisher
claims when they are saved, so the first OIDC publish remains the authoritative
end-to-end check. Any configuration drift makes publication fail closed.
