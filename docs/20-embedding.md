...
|---|---|---|---|
| **1 Creator** | Valid bearer matching `webTransport.auth.token`, AND no `x-visitor-token` header, AND no valid `x-agent-id`+`x-agent-secret` pair | `creator` | hardcoded `"creator"` |
...
| **4 Public / anonymous** | Default — fallback when above don't match. Includes (a) admitted-by-`allowAnonymous` with no bearer, (b) bearer-validated with a stale or malformed `x-visitor-token` (visitor-token presence wins routing — see warning below). | `public` + `anonymous` | `"anon-" + threadId` |

> ⚠ **Mixed bearer + visitor-token is a footgun.** The runtime treats any present `x-visitor-token` (even stale/garbled) as an explicit "I am acting as a visitor" signal — Path 1 (creator) only fires when `x-visitor-token` is **absent**. A bearer-credentialed caller who forwards a stale visitor token gets silently downgraded to `public/anonymous`: anonymous budget caps apply, memory writes go to the `anon-*` namespace, creator-only tools become inaccessible. **Adopters must not mix headers in real client code.** Auggy's shipped surfaces (`auggy chat`, `telegramTransport`) don't mix; widgets you build shouldn't either. Pinned by `tests/integration/embedding-primitives.test.ts` ("valid bearer + stale x-visitor-token → public/anonymous"). Filed for a v1.x design discussion at `docs/todos.md` G38 (bearer-precedence).
