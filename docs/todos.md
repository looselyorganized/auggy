...
- [ ] **[transport] Flaky webTransport tests under suite-wide load.** Two tests in `tests/transports/web-transport.test.ts` have intermittently failed in CI and locally when the full suite runs, then passed cleanly on isolated retry: `Fix 1: missing visitor token + visitorTokens enabled stays anonymous on first request` and `Fix 1: invalid visitor token does NOT promote request to public:recognized`. Failure mode is "Expected: 200" — an HTTP assertion against a server that may not have fully bound its port before the client request, or that's reusing a port from a prior test. Suspects: per-test `Bun.serve()` startup/teardown isn't fully synchronous, or cross-test cleanup leaks. Reproduces ~1 in 10 runs locally. Surfaced 2026-04-28 during CI for the bash process-group fix.
