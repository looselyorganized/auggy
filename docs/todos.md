...
- [ ] **[posture]** `webTransport.allowAnonymous` resolution + boot log + visitor-auth-missing warning already shipped. **Remaining work: deterministic unit-test coverage** for the gating semantics — missing-bearer + `allowAnonymous: false` → 401; yaml-explicit-true → admits; env-default-true with no visitor-auth → warning emitted; bearer present-but-invalid → 401 (no silent downgrade, already covered by `tests/integration/embedding-primitives.test.ts` test #6). LLM-judged eval expansion deferred to v1.0 ship (see note at section end). (G3, ~30 min unit tests)
...
- [ ] **[admin]** `/admin` route + OpenHands-style HTTP basic auth, bearer-as-password — native browser prompt, no new credential or env var (reuses `AUGGY_WEB_TOKEN`). Accept `Authorization: Basic <base64(":" + bearer)>` alongside the existing Bearer header; 401 + `WWW-Authenticate: Basic realm="auggy-admin <agent-name>"` triggers the browser prompt. Per-route rate limit declared explicitly (NOT inherited — `webTransport.checkRouteRateLimit` only fires on routes that declare it, verified at `src/transports/web-transport.ts`). Mandates HTTPS in production: refuses plain HTTP on non-loopback interfaces (mirrors G3's posture gate). Operator must configure `webTransport.trustedProxies` on cloud deploys or per-IP limits collapse to a global cap (Railway/Fly's edge proxy IP becomes the only seen IP). Auth check lives in the admin augment handler (NOT in `webTransport` core, mirroring `visitorAuth`'s per-route auth-extract pattern at `src/augments/visitor-auth/index.ts`). Optional `auggy admin <name>` CLI verb resolves agent URL from `~/.auggy/agents.json` (cloud record preferred, falls back to local PID manifest at `~/.auggy/<name>.json`). Dashboard content absorbs G9 spend visibility, G14 memory inspection, G19 notify test, G3 anonymous-posture toggle. Threat model documented: browser autofill on shared computers, no clean logout, URL-embedded credentials, TLS dependency, devtools exposure. (G36, ~3-4 days)
