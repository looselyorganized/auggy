...
- [ ] **[chat]** Bake a minimal `/chat` HTML page into `webTransport`, served at `GET /` when no `publicFrontendUrl` is set. Localhost-default-on; public deploys gate behind visitorAuth's verify-email flow. Self-contained HTML+JS speaking AG-UI to `/agent/run`. (G2, ~half-day)
