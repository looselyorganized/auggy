...
| **Double-start prevention** | Two `aug1 dev zip` → second fails with clear error | Boot agent, attempt second boot, assert EEXIST error |
...
Before building `aug1 eval run`, test these manually against a running Zip agent:
...
- Judge calibration workflow: `aug1 eval calibrate`
- Paired with 25-50 human-labeled reference set

### Phase 3: CI Integration + Trace Viewer
- `aug1 eval run --ci` with exit code 1 on regression failures
- `aug1 trace view <run-id>` for event replay
...
Every commit:
  ...
  → aug1 eval run eval/regression/ (20-50 cases, Pass^3 ≥95%)

Every feature:
  → aug1 eval run eval/capability/ (broader suite, Pass^3 tracked but not gated)
