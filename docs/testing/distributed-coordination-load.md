...
The model actively sends one stale-fence and one cross-namespace read probe through its reference seams. Mutation idempotency is scoped by namespace, and the order-support profile deliberately exercises the same raw mutation key in more than one namespace. Its test-only fault settings can deliberately break those seams or simulate unavailable/unknown outcomes; a nonzero result must fail the default threshold evaluation. This makes the output a model check, not evidence that production storage or a networked coordinator rejected the same operation.
