...
The JSON result contains only bounded aggregate metrics: the requested replica count, throughput, active and queued peaks, p50/p95/p99 queue wait, rejections, unavailable and outcome-unknown counts, duplicate mutations, same-thread overlap, stale-fence accepts, and namespace violations. It never accepts provider credentials or production traffic data.
