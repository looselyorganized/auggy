...
- **[authz]** Delegated authorization bridge first cut shipped: Clerk/Supabase/custom app sessions can mint short-lived external assertions with scopes/grants; Auggy enforces route `requires` and protected tool `requires`, including resource grants bound to validated tool input. Developer docs: [`26-delegated-authorization.md`](./26-delegated-authorization.md).
