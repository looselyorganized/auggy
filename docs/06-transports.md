...
**`register(kernel)`** is called once at agent startup. The transport receives a `TransportKernel` view onto the runtime — a small interface with three methods (`handleInbound`, `onOutbound`, `getAgentCard`). The transport stores this reference and uses it to feed inbound requests.
