---
name: manifest
description: When and how to use manifest_fetch to retrieve knowledge from the operator's organization. Read this before answering questions about the org or its work.
---

# Manifest

You are connected to your operator's **organization knowledge base** via a manifest — a list of information endpoints the org exposes (paths like `/mission`, `/team`, `/projects`). The manifest itself is already in your system context. Use `manifest_fetch` to pull the actual content from any of those endpoints when the conversation needs it.
...
| Tool | What it does |
|------|--------------|
| `manifest_fetch(endpoint)` | Retrieve the content of one endpoint listed in the manifest |

Only `manifest_fetch` is exposed here. If you need to alert the operator out-of-band about something (escalate a request you can't handle, flag urgent input, ask for human approval), that's a separate capability — see the `notify` skill if it's mounted in this agent. Don't try to use `manifest_fetch` for it.

## How to call it

```
manifest_fetch({ endpoint: "/mission" })
```

Or with an optional prompt that the tool can pass through to influence what comes back:

```
manifest_fetch({ endpoint: "/projects", prompt: "anything related to onboarding" })
```
...
| Peer asks about a specific topic (mission, team, projects, decisions) | Find the matching endpoint in the manifest and `manifest_fetch` it |
...
The manifest in your system context lists the endpoints available — read it before calling `manifest_fetch` so you choose the right one. If the manifest doesn't list an endpoint covering what the peer wants, say so directly rather than guessing at paths that may not exist.
...
- You cannot use `manifest_fetch` to reach arbitrary URLs on the public web — for that, use the `web_fetch` tool if it's mounted.

## Common mistakes

| Wrong | Correct |
|-------|---------|
| Saying "I don't know what this organization is" | Check the manifest already in your context; `manifest_fetch` a relevant endpoint if details are needed |
...
| Using `manifest_fetch` for general web pages | Use `web_fetch` for arbitrary URLs; `manifest_fetch` is scoped to the operator's org API |
...
2. If yes, answer from the summary. If no, find the most relevant endpoint and `manifest_fetch` it.
