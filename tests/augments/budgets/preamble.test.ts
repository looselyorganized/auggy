...
describe("buildBudgetPreamble", () => {
  ...
  it("computes the minimum ratio across all configured caps", () => {
    ...
    const block = buildBudgetPreamble(input(
      { maxTurnsPerThread: 10, maxTurnsPerDay: 10 },
      { thread: 6, day: 2, costUsd: 0 },
    ));
  ...
  it("emits all configured cap lines when all three caps are set", () => {
    const block = buildBudgetPreamble(input(
      { maxTurnsPerThread: 20, maxTurnsPerDay: 50, maxUsdPerDay: 1.00 },
