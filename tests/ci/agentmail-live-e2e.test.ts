...
describe("AgentMail protected live mutation E2E", () => {
  ...
  test("exercises Auggy's live runtime and cleans up its temporary fixture", () => {
    ...
    expect(script).toContain("const canonicalDelivery = catchUpDelivery ?? mailboxDelivery");
    expect(script).toContain("return canonicalDelivery");
    ...
    expect(script).toContain("updateRuntimeDiagnostics");
    expect(script).toContain("cleanupCurrentRun");
    expect(script).toContain("subject: [subject]");
    expect(script).toContain("for (const subject of [input.subject, input.replySubject])");
    expect(script).toContain("draft.subject === input.replySubject");
