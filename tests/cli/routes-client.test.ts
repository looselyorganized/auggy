...
function report(): ClientRoutesReport {
  return {
    ...
    summary: {
      ...
      publicRoutes: 2,
      privateRoutes: 3,
      publicRoutePaths: ["GET /services/:serviceId", "POST /webhooks/stripe"],
    },
    routes: [
      {
        method: "GET",
...
function practicalBrowserUsageFixture(): string {
  return `
    ...
    // @ts-expect-error browser target omits webhook-policy routes.
...
function practicalServerUsageFixture(): string {
  return `
    ...
    api.get("/agent-api/search", { query: { q: "inventory" } });
    api.post("/webhooks/stripe", { headers: { "stripe-signature": "sig" } });
...
describe("createTypeScriptClient", () => {
  test("generates a browser TypeScript client from a route manifest by default", () => {
    ...
    expect(source).toContain("Browser clients omit creator/bearer/agent and webhook-policy routes");
    expect(source).not.toContain('from "auggy"');
    ...
    expect(source).not.toContain('"/webhooks/stripe":');
    ...
    expect(source).toContain("* - POST /webhooks/stripe auth=none policy=webhook.signature:stripe");
  ...
  test("generates a server TypeScript client with bearer routes", () => {
    ...
    expect(source).toContain(
      "Server clients include creator/bearer, agent, and webhook-policy routes",
    );
    expect(source).toContain(
  ...
  test(
    ...
    async () => {
      await expectGeneratedClientTypechecks(
        ...
        // @ts-expect-error browser target omits webhook POST result paths.
        type WebhookResult = AuggyPostResult<"/webhooks/stripe">;
      ...
      await expectGeneratedClientTypechecks(
        ...
        async function main() {
          ...
          const webhook: AuggyPostResult<"/webhooks/stripe"> = await api.post(
            "/webhooks/stripe",
            { headers: { "stripe-signature": "sig" } },
          );
          if (webhook.ok) {
            const webhookData: unknown = webhook.data;
  ...
  test("browser generated runtime sends params, query, and visitor auth headers", async () => {
    ...
    await expect(api.post("/webhooks/stripe")).rejects.toThrow(
      "Unknown Auggy route: POST /webhooks/stripe",
    );
  ...
  test("server generated runtime sends JSON bodies and bearer auth headers", async () => {
    ...
    const webhookResult = await api.post("/webhooks/stripe", {
      headers: { "stripe-signature": "sig" },
    });
    ...
    expect(webhookResult.data).toEqual({
      requestUrl: "https://agent.example/webhooks/stripe",
    });
    expect(calls[2]?.url).toBe("https://agent.example/webhooks/stripe");
    expect(new Headers(calls[2]?.init.headers).get("stripe-signature")).toBe("sig");
