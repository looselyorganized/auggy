...
- **`visitorAuth.allowConsoleInProduction` option** (default `false`). When `NODE_ENV === "production"` AND `agentMail.transport === "console"`, the factory rejects at boot with a clear error message — magic links would otherwise be written to runtime logs (Railway / Fly dashboards) where anyone with log access could harvest them. Operator must set `allowConsoleInProduction: true` to explicitly acknowledge the risk.
