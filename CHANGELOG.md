...
- **`docs/20-embedding.md` hardening pass** (codex round-2 follow-up): verify reverse-proxy now uses an explicit header allowlist (only `content-type` forwarded; cookies, authorization, and other frontend-session material stripped); Pattern A proxy server-mints `threadId` and binds via signed HttpOnly cookie (browser-supplied threadId is IGNORED — closes the namespace-collision attack we'd documented but not mitigated); both proxies forward `Idempotency-Key` with accurate docs on what the runtime actually dedups (budget reservations only, NOT turn results — the model is re-called on retry, tools re-execute). New env var `AUGGY_SESSION_SECRET` for the cookie signature (generate via `openssl rand -hex 32`). 3 new regression tests in `tests/integration/embedding-recipe.test.ts` (signed-cookie continuity, tampered-cookie rejection, idempotency truth).

- **`docs/20-embedding.md` — copy-paste recipe** for wiring a Next.js (or any server-rendered) chat surface to a running Auggy agent. Documents two patterns: **Pattern A** for public visitor chat (no bearer in proxy, `allowAnonymous: true` on agent, visitor-token rotation, visitorAuth upgrade with reverse-proxied verify route) and **Pattern B** for operator-only surfaces (bearer forwarded → creator trust, explicit warning never to expose publicly). Includes the corrected identity model (clarifies that `x-peer-id` is NOT used for identity), CORS clarification (it's a response-header signal, not an auth gate), cross-origin deployment guidance (reverse-proxy `/visitor-auth/verify` for shared-localStorage handoff), and a hardening checklist covering credential-leak and auth-workaround vectors. Verified end-to-end by `tests/integration/embedding-recipe.test.ts` (6 integration tests proving the documented claims hold against the real transport). (G1 — v1.0 concierge-readiness.)
- **`tests/integration/embedding-recipe.test.ts`** — integration suite that boots a real agent + small inline HTTP proxy and asserts that Pattern A stays in `public/anonymous` → `public/recognized` (NOT creator), that the visitor-token rotation flow works end-to-end, that `x-peer-id` is ignored for identity, and that Pattern B's bearer-forwarding correctly maps to `creator` trust. Regression guard against future doc changes that drift from the transport contract.
