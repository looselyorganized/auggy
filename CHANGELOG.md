...
- **`docs/20-embedding.md` — copy-paste recipe** for wiring a Next.js (or any server-rendered) chat surface to a running Auggy agent. Three files (server proxy route, React widget, page), two env vars, full AG-UI event-stream handling, security notes, and a troubleshooting section. Closes the OSS gap where adopters could read `docs/06-transports.md` to learn the wire protocol but had no concrete copy-paste integration example. (G1 — v1.0 concierge-readiness.)
