...
- **`docs/20-embedding.md` mixed-header footgun callout** (codex round-6 follow-up) — the doc now explicitly warns that mixing bearer + `x-visitor-token` silently demotes a creator to anonymous (visitor-token presence wins routing). New test in `tests/integration/embedding-primitives.test.ts` ("valid bearer + stale x-visitor-token → public/anonymous") pins the current runtime behavior so any future change to bearer-precedence will need to flip it. Filed for design revisit at `docs/todos.md` G38. Also fixed a contradiction in the visitor-token rotation section ("missing/invalid → mints" → unambiguous "header MUST be present, any non-empty value works as bootstrap sentinel"). No runtime change in this PR.
- **Path 1 doc clarification** — the identity-paths table now reads "no valid `x-agent-id`+`x-agent-secret` pair" instead of the looser "no `x-agent-*` headers" wording (codex round-6 precision nit on the trigger condition).
