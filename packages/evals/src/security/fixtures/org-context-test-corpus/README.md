# orgContext test corpus

This directory is a synthetic org-knowledge corpus consumed by the security
eval fixture via the orgContext augment's `file://` baseUrl scheme.
...
suite a concrete `org_fetch` surface to test against (benign-suite cases
that exercise legitimate org-info questions, plus any future adversarial
cases that probe the orgContext attack surface).

## Layout

- `manifest` — JSON listing of advertised endpoints (no extension; the
  orgContext file:// scheme reads this file literally as `manifest`)
...
their own org content can either replace these files or point orgContext
