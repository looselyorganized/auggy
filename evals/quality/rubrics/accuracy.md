...
- **2 — Fully correct and grounded.** Response accurately reflects information from tool results (org_fetch, memory_read) or established identity. No fabrication. Key facts present and precise. Examples: accurately describes LORF as a research facility with AI agents, correctly references the facility's purpose and vision.

## Notes for the judge
- If the agent called a tool (org_fetch, memory_read) and the response reflects the tool's output, that's grounding — grade favorably.
